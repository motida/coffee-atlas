...
Eight edge tables are defined in the schema to materialize these relationships for graph traversal. Three are populated from data today (`edges_country_region`, `edges_region_farm`, and the variety↔flavor / origin↔variety edges built by the CQI and graph stages); the rest are schema-ready and fill in as their source stages land. Origin → Variety is split per origin level so each edge gets a real foreign key (no polymorphic IDs):
