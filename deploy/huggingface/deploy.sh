...
# Bring a persistent staging clone up to date with the Space (or create it).
# CI pushes backend code to the api Space on every merge to main, so a stale
# clone makes the next deploy commit non-fast-forward and the push is
# rejected. The tree is fully regenerated by rsync/cp below, so a hard reset
# to the remote loses nothing.
refresh_stage() {
    local stage="$1" url="$2"
    if [[ ! -d "$stage/.git" ]]; then
        rm -rf "$stage"
