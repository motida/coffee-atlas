...
Key properties: `roastedBy` (→ Roaster), `roastLevel`, `process`, `isBlend`, `price`, `priceCurrency` (ISO 4217 — storefront prices are scraped in each store's local currency), `netWeightGrams`, `consistsOf` (→ Variety), `hasFlavor` (→ FlavorAttribute), `fromOrigin` (→ Country/Region)
...
OVERTURE_RELEASE=2026-07-22.0      # Overture release for the shops stage (old releases age out of S3)
