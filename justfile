...
# Run a specific ingest stage (lexicon, varieties, cqi, processing_descriptions, processing_flavor, geocode, shops, descriptions, distribution, roasting, products, roaster_locations, embeddings, graph, specialty, roaster_discovery)
ingest stage:
    uv run python -m backend.ingest.pipeline --stage {{ stage }}

# Run the full ingest pipeline in order (shops + descriptions + products excluded: network-heavy, see docs/DATA.md)
ingest-all:
    just ingest lexicon
